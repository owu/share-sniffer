...
// 返回值:
// - Result: 包含检查结果的结构体
func (u *UcChecker) Check(urlStr string) Result {
	return u.checkUc(urlStr)
}
...
func (u *UcChecker) checkUc(urlStr string) Result {
	logger.Debug("UcChecker:开始检测UC网盘链接: %s", urlStr)

	// 创建带超时的context - 确保请求不会无限等待
	timeout := config.GetHTTPClientTimeout()
	logger.Debug("创建带超时的context，超时时间: %v", timeout)
	ctx, cancel := context.WithTimeout(context.Background(), timeout)
	defer cancel() // 确保在函数退出时取消context，防止资源泄漏

	// 提取资源ID - 解析URL中的关键参数
	code, err := extractCodeFromUcURL(urlStr)
	if err != nil {
		logger.Info("UcChecker:extractCodeFromUcURL,%s,错误: %v\n", urlStr, err)
		return Result{
			URL:    urlStr,
			Name:   "链接格式无效",
			Status: 0,
	...
	if response.Status == 200 && response.Code == 0 {
		...
		return Result{
			URL:     urlStr,
			Name:    response.Data.DetailInfo.Share.Title,
			Status:  1,
			Elapsed: requestElapsed,
		}
	} else {
...
func ucRequest(ctx context.Context, code string) (*ucResp, error) {
	...
	logger.Debug("获取UC网盘API响应成功: Status=%d, Code=%d, Message=%s", response.Status, response.Code, response.Message)
	return &response, nil
}
...
// extractCodeFromUcURL 从UC网盘链接中提取code参数
...
// - code: 提取的code参数
// - error: 发生的错误，如果有
func extractCodeFromUcURL(rawURL string) (code string, err error) {
