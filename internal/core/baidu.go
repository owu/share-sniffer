...
// 返回值:
// - Result: 包含检查结果的结构体
func (q *BaiduChecker) Check(urlStr string) Result {
	return q.checkBaidu(urlStr)
}
...
func (q *BaiduChecker) checkBaidu(urlStr string) Result {
	...
	if err != nil {
		return Result{
			Name:   fmt.Sprintf("解析基础URL失败: %v", err),
			Status: 0,
		}
	}
	...
	result.Status = 1
	result.Name = step3Result.JSONResponse.Title

	return result // 返回完整的检查结果
}
...
func step1Request(targetURL string) (*Step1Response, error) {
	client := &http.Client{
...
// 第二步请求：验证请求
func step2Request(step1Result *Step1Response, password string) (*Step2Response, error) {
	...
	req, err := http.NewRequest("POST", fullURL, bytes.NewBufferString(postData.Encode()))
	if err != nil {
...
// 第三步请求：获取文件列表
func step3Request(step1Result *Step1Response, step2Result *Step2Response) (*Step3Response, error) {
	jar, err := cookiejar.New(nil)
	if err != nil {
		return nil, fmt.Errorf("创建Cookie Jar失败: %v", err)
	}
