...
func (q *CheckUI) createHeaderContainer() *fyne.Container {
	...
	noteHeaderLabel := widget.NewLabel("备注")
...
func (q *CheckUI) createDataTable(tableData [][]string, mutex *sync.RWMutex) *container.Scroll {
	...
	// 自定义表头
	headers := []string{"序号", "网址", "状态", "耗时ms", "备注"}
...
func (q *CheckUI) loadToTable() {
	...
	// 将表格数据保存到CheckUI实例中，方便CheckFile方法访问
	// 为了避免并发问题，我们创建一个包含锁的数据结构
	logger.Debug("创建表格数据包装器")
	...
	dataTableContainer := q.createDataTable(tableDataWrapper.Data, &tableDataWrapper.Mutex)
...
func (q *CheckUI) CheckFile() {
	...
	for i := 0; i < len(links); i++ {
		tableData[i] = []string{fmt.Sprintf("%d", i+1), links[i], "检测中", "", ""}
	}
	var tableDataMutex sync.RWMutex
	logger.Debug("表格数据初始化完成，共 %d 行数据", len(tableData))
	...
	fyne.Do(func() {
		...
		// 创建表头和数据表格
		headerContainer := q.createHeaderContainer()
		dataTableContainer := q.createDataTable(tableData, &tableDataMutex)
	...
	for batchStart := 0; batchStart < totalLinks; batchStart += batchSize {
		...
		for i := batchStart; i < batchEnd; i++ {
			...
			task := func(ctx context.Context) interface{} {
				...
				// 首先更新状态为检测中（确保UI显示正确）
				tableDataMutex.Lock()
				if tableData[index][2] != "已停止" {
					tableData[index][2] = "检测中"
				}
				...
				case <-q.stopChan:
					...
					return taskResult{index: index, result: core.Result{Status: -2}} // -2表示已停止
				case <-ctx.Done():
					logger.Debug("任务 #%d 上下文已取消", index+1)
					return taskResult{index: index, result: core.Result{Status: -2}} // 上下文取消也视为停止
				default:
					// 继续检测
				}
				...
				// 根据任务数量调整日志级别
				if totalLinks < 1000 {
					logger.Debug("任务 #%d 检测完成，状态: %d, 耗时: %v", index+1, result.Status, time.Since(taskStartTime))
				} else {
					logger.Debug("任务 #%d 检测完成，状态: %d, 耗时: %v", index+1, result.Status, time.Since(taskStartTime))
				}
				...
				case <-q.stopChan:
					...
					return taskResult{index: index, result: core.Result{Status: -2}}
				case <-ctx.Done():
			...
			if !submitSuccess {
				...
				go func(idx int) {
					tableDataMutex.Lock()
					defer tableDataMutex.Unlock()
					if tableData[idx][2] == "检测中" {
						tableData[idx][2] = "失败"
						tableData[idx][4] = "任务提交失败"
					}
...
stopSubmission:
	...
	go func() {
		...
		// 批量刷新UI计数器，避免每处理一个任务就刷新一次UI
		uiRefreshCounter := 0
		uiRefreshInterval := 10 // 每处理10个任务刷新一次UI
		...
		for result := range resultsChan {
			...
			// 检查索引是否有效
			if index < 0 || index >= len(tableData) {
			...
			if tableData[index][2] != "已停止" {
				if checkResult.Status == -2 {
					tableData[index][2] = "已停止"
					logger.Debug("任务 #%d 已停止", index+1)
				} else {
					statusText := "未知"
					if checkResult.Status == 1 {
						statusText = "正常"
						logger.Debug("任务 #%d 检测正常: %s", index+1, checkResult.Name)
					...
					tableData[index][2] = statusText
					tableData[index][3] = fmt.Sprintf("%d", checkResult.Elapsed)
					tableData[index][4] = checkResult.Name
				}
			}
			tableDataMutex.Unlock()
			...
			// 增加UI刷新计数器
			uiRefreshCounter++
			// 批量刷新UI，调整刷新策略确保更及时的UI更新
			if uiRefreshCounter >= 1 || atomic.LoadInt32(&completedCount) == int32(totalTasks) {
				fyne.Do(func() {
					// 对于大量任务，降低UI刷新日志的详细程度
					if totalTasks < 1000 {
						logger.Debug("更新UI：刷新表格显示任务 #%d 结果", index+1)
					} else if uiRefreshCounter >= uiRefreshInterval*5 {
						logger.Debug("批量更新UI：已处理 %d/%d 个任务", atomic.LoadInt32(&completedCount), totalTasks)
					}
					if q.resultTable != nil {
						q.resultTable.Refresh()
