...
プロジェクトには、`/package`ディレクトリに自動化パッケージ化スクリプトが提供されており、WindowsおよびLinuxプラットフォームのパッケージ化をサポートしています。

### 5.1 パッケージ化スクリプトの説明

| スクリプト名 | プラットフォーム | 説明 |
|---------|------|------|
| `build-windows.ps1` | Windows | WindowsプラットフォームのGUI実行ファイルを構築するためのPowerShellスクリプト |
| `build-linux.sh` | Linux | LinuxプラットフォームのGUIインストールパッケージを構築するためのBashスクリプト |
...
- 生成されたファイルに自動的に名前を付け、`/package`ディレクトリに出力する
...
1. 最新のインストールパッケージ`ShareSniffer.v0.1.0.linux-amd64.tar.xz`を任意のディレクトリにダウンロードします
...
```
最新のshare-sniffer-cli.v0.1.0.linux-amd64をダウンロードします
...
```
最新のshare-sniffer-cli.v0.1.0.windows-amd64.exeをダウンロードします
...
| フィールド | タイプ | 説明 |
|------|------|------|
| `error` | int | エラーコード、0は成功、0以外は失敗を示す |
| `msg` | string | ステータス説明、"success"は成功、"failed"は失敗、"timeout"はタイムアウトを示す |
| `data` | object | 検出結果の詳細 |
