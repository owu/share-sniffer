...
项目提供了自动化打包脚本，位于 `/package` 目录下，支持 Windows 和 Linux 平台的打包。

### 5.1 打包脚本说明

| 脚本名称 | 平台 | 说明 |
|---------|------|------|
| `build-windows.ps1` | Windows | PowerShell脚本，用于构建Windows平台的GUI可执行文件 |
| `build-linux.sh` | Linux | Bash脚本，用于构建Linux平台的GUI安装包 |
...
1. 下载最新安装包 `ShareSniffer.v0.1.0.linux-amd64.tar.xz` 到任意目录
...
#### 6.3.1 Linux 安装
```
下载最新版 share-sniffer-cli.v0.1.0.linux-amd64
重命名为 share-sniffer-cli 
可执行文件移动到 `/usr/local/bin` 目录
```
#### 6.3.2 Windows 安装
```
下载最新版 share-sniffer-cli.v0.1.0.windows-amd64.exe
...
| 字段 | 类型 | 说明 |
|------|------|------|
| `error` | int | 错误码，0 表示成功，非0表示失败 |
| `msg` | string | 状态描述，"success" 表示成功，"failed" 表示失败，"timeout" 表示超时 |
| `data` | object | 检测结果详情 |
