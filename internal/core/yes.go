...
// 返回值:
// - Result: 包含检查结果的结构体
func (y *YesChecker) Check(urlStr string) Result {
	return y.checkYes(urlStr)
}
...
func (y *YesChecker) checkYes(urlStr string) Result {
	...
	return Result{
		URL:     urlStr,
		Name:    response.Info.Data.ShareName,
		Status:  1,
		Elapsed: requestElapsed,
	}
}
