...
// 返回值:
// - Result: 包含检查结果的结构体
func (q *YywChecker) Check(urlStr string) Result {
	return q.checkYyw(urlStr)
}
...
func (q *YywChecker) checkYyw(urlStr string) Result {
	logger.Debug("YywChecker:开始检测115网盘链接: %s", urlStr)

	// 创建带超时的context - 确保请求不会无限等待
	timeout := config.GetHTTPClientTimeout()
	logger.Debug("YywChecker:创建带超时的context，超时时间: %v", timeout)
	ctx, cancel := context.WithTimeout(context.Background(), timeout)
	defer cancel() // 确保在函数退出时取消context，防止资源泄漏

	// 提取参数
	shareCode, receiveCode, err := extractParams(urlStr)
	if err != nil || shareCode == "" || receiveCode == "" {
		logger.Warn("YywChecker:extractParams,%s,错误: %v\n", urlStr, err)
		return Result{
			Name:   "链接格式无效",
			Status: 0,
		}
	...
	if err != nil {
		...
		if errors.IsTimeoutError(err) {
			logger.Warn("YywChecker:请求超时: %s, 请求耗时: %dms", urlStr, requestElapsed)
			return Result{
				Name:   "请求超时",
				Status: -1,
			}
		}
		logger.Warn("YywChecker:检测失败: %s, 错误: %v, 请求耗时: %dms", urlStr, err, requestElapsed)
		return Result{
			Name:   "检测失败: " + err.Error(),
			Status: 0,
		}
	...
	if response.State && response.Errno == 0 {
		...
		if response.Data.Shareinfo.ShareTitle != "" {
			result.Name = response.Data.Shareinfo.ShareTitle
		} else if len(response.Data.List) > 0 && response.Data.List[0].N != "" {
			result.Name = response.Data.List[0].N
		}
		// 处理Unicode转义序列（如\u86df\u9f99\u884c\u52a8）[6](@ref)
		result.Name = unicodeToChinese(result.Name)
	} else {
		result.Status = 0
	}

	logger.Debug("YywChecker:检测成功: %s, 文件名: %s, 请求完成: %dms", urlStr, result.Name, requestElapsed)
...
// extractParams 从URL中提取share_code和receive_code[3](@ref)
func extractParams(urlStr string) (shareCode, receiveCode string, err error) {
