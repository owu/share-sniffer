...
// 返回值:
// - Result: 包含检查结果的结构体
func (q *TelecomChecker) Check(urlStr string) Result {
...
func checkTelecom(urlStr string) Result {
	...
	if response.ResCode == 0 && response.ResMessage == "成功" {
		...
		result.Name = response.FileName // 设置文件名
	} else {
		result.Status = 0 // 失败状态
...
// extractCodeFromURL 从URL中提取code参数，自动处理编码状态
