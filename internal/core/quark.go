...
// 返回值:
// - Result: 包含检查结果的结构体
func (q *QuarkChecker) Check(urlStr string) Result {
	return q.checkQuark(urlStr)
}
...
func (q *QuarkChecker) checkQuark(urlStr string) Result {
...
func quarkRequest(ctx context.Context, resourceID string, passCode string) (*quarkResp, error) {
	...
	logger.Debug("获取夸克API响应成功: Status=%d, Code=%d", response.Status, response.Code)
	return &response, nil
}
